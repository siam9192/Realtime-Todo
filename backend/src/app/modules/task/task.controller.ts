...
createTask = catchAsync(async (req, res) => {
  ...
  if (data.assignedToId) {
    notificationService.instantNotifyToUsers([data.assignedToId], notificationPayload);
...
4  assignment changed?
   ↓
5  UNASSIGN (from) → notify + socket
   ↓
6  ASSIGN (to) → notify + socket
   ↓
7  emit task:updated
   ↓
8  HTTP response */
  ...
  // Emit task updated event to creator and if assignment no changes  then assigned user also
   emitToUsers((assigned ? [data.creatorId] :[data.creatorId,data.assignedToId]).filter(_=>_ !== null),TaskEvent.UPDATED,{ id: data.id });
