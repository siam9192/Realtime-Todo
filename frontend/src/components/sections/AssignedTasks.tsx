...
import { useGetAssignedTasksQuery } from "../../query/services/task.service";
...
function AssignedTasks() {
  ...
  const { data, isLoading, refetch } = useGetAssignedTasksQuery(params);
  ...
  return (
    <section>
      ...
      {showResult ? (
        <>
          {meta?.totalResults === 0 ? (
            <p className="text-sm opacity-70">No assigned tasks found.</p>
          ) : (
